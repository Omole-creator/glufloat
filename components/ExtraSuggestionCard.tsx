 ...
 * and a calorie count.
 *
 * `set.items` is EVERY serving recommended for this meal, together — not 3
 * alternatives to pick one of. `lib/nextMeal.ts`'s `suggestExtras()` already
 * sizes this list so the main plate plus every item here sums to this
 * meal's own fair share of the day (founder instruction, 2026-08-31: "all
 * recommended meals must add up at the end of the day to meet each user
 * calorie goals"), so there is no guessing how many times to come back and
 * log another snack — the right amount is already on screen. "I ate this
 * too" logs the WHOLE list as one entry.
 *
 ...
 * the gap), the list simply appears again; nothing here silently blocks a
 * second, real log.
 *
...
/**
 * A big gap can need more servings than there are distinct real options for
 * a meal (only 3), so the same safe food may appear more than once in
 * `set.items` — grouped here into one row with a "×N" count, same house rule
 * as `lib/shareMessage.ts` deduping a repeated `healthNote`: showing the same
 * name, portion line and caution 3 times over is noise, not information.
 */
function groupExtras(set: ExtraSuggestionSet): GroupedExtra[] {
...
export default function ExtraSuggestionCard({ set }: { set: ExtraSuggestionSet }) {
  const grouped = groupExtras(set);

  const logEaten = () => {
    const allFoods = set.items.flatMap((o) => o.foods);
    ...
    <div className="rounded-2xl bg-leaf/5 p-4 ring-1 ring-inset ring-leaf/20">
      ...
      <div className="mt-3 flex items-center justify-between border-t border-leaf/15 pt-3">
        <p className="text-sm font-semibold text-ink-soft">
          Total: <span className="text-ink">{set.totalCalories} kcal</span>
        </p>
        <button
          type="button"
          onClick={logEaten}
