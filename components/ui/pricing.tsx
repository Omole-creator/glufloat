      ...
      >
        {plans.map((plan) => (
          <motion.div
            key={plan.tier}
            variants={card}
            className={`relative flex flex-col overflow-hidden rounded-3xl bg-white p-6 text-center shadow-[0_20px_50px_-24px_rgba(12,42,71,0.35)] transition-all hover:-translate-y-1 ${
              plan.isPopular ? "border-2 border-brand/30" : "border border-line"
            }`}
          >
            {plan.isPopular && (
              <div className="absolute right-0 top-0 flex items-center gap-1 rounded-bl-xl bg-brand px-3 py-1">
            ...
            <ul className="mx-auto mt-6 flex-1 space-y-3 text-left text-sm text-ink">
              {plan.features.map((f) => (
            ...
            <TrialCta
              className={`group mt-6 flex items-center justify-center gap-2 rounded-full px-6 py-3.5 text-sm font-bold text-white transition-all hover:-translate-y-0.5 ${
                plan.isPopular
            ...
            <p className="mt-3 text-xs text-ink-soft">{plan.description}</p>
          </motion.div>
        ))}
      </motion.div>
    </div>
