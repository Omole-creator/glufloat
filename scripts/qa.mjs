...
ok("disclaimer gate visible on first open", await page.getByText("I understand. Show me the food answers.").isVisible());
await page.screenshot({ path: `${OUT}/app-disclaimer.png` });
await page.getByText("I understand. Show me the food answers.").click();
...
ok("eba verdict yellow", await page.getByText("YELLOW. Yes, with care.").isVisible());
...
const yellowVisible = await page.getByText("Close. A small fix turns this green.").isVisible();
ok("meal starts yellow (normal rice + veg + fish)", yellowVisible);
await page.getByRole("button", { name: "Half" }).click();
await page.waitForTimeout(400);
ok("half portion turns meal green", await page.getByText("This plate works. Enjoy it.").isVisible());
...
await page.locator("ul button").first().click();
await page.waitForTimeout(400);
ok("coke locks meal red", await page.getByText("This meal is red because of the sugary drink.").isVisible());
